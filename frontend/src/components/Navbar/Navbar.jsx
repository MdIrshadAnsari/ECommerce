import React from 'react'
import {Link} from "react-router-dom"

const Navbar = () => {
    const links = [
        {
        title: "HOME",
        link: "/"
    },
     {
        title: "COLLECTION",
        link: "/collection"
    },
     {
        title: "ABOUT",
        link: "/about"
    },
    {
        title: "CONTACT",
        link: "/contact"
    },
]
  return (
    <div className='flex justify-around h-20 items-center'>
        <div className='w-[8vw] flex items-center'>
            <img src="data:image/png;base64," alt=""/>
        </div>
        <div className='mid-div flex gap-5 font-semibold text-zinc-700 text-sm'>
            {links.map((item, index)=>(
                <div>
                    <Link to={item.link}>{item.title}</Link>
                </div>
            ))}
        </div>
        <div className='flex items-center gap-5'>
            <Link to="/product/collection">
             <img className='w-5 h-5' src="data:image/png;base64," alt="" />
            </Link>
           
            <img className='w-5 h-5' src="data:image/png;base64," alt="" />
            <img className='w-5 h-5' src="data:image/png;base64," alt="" />
        </div>
    </div>
  )
}

export default Navbar