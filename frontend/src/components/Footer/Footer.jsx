import React from "react";

const Footer = () => {
  return (
    <footer className="w-full mt-12 px-6 py-10">
      <div className="flex justify-between gap-10 items-start">
        
        {/* Left Section */}
        <div className="flex flex-col gap-4">
          <img
            src="data:image/png;base64,"
            alt="Company Logo"
            className="w-24"
          />
          <p className="text-sm font-semibold text-zinc-700 leading-relaxed">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptas
            mollitia natus ipsam similique harum.
          </p>
        </div>

        {/* Middle Section */}
        <div className="flex flex-col gap-4">
          <h2 className="text-xl font-semibold">Company</h2>
          <nav className="flex flex-col gap-2 ">
            <a href="#" className="text-sm text-zinc-600 hover:text-black font-semibold">Home</a>
            <a href="#" className="text-sm text-zinc-600 hover:text-black font-semibold">About Us</a>
            <a href="#" className="text-sm text-zinc-600 hover:text-black font-semibold">Delivery</a>
            <a href="#" className="text-sm text-zinc-600 hover:text-black font-semibold">Privacy Policy</a>
          </nav>
        </div>

        {/* Right Section */}
        <div className="flex flex-col gap-3">
          <h2 className="text-xl font-semibold">Get in Touch</h2>
          <p className="text-sm text-zinc-700 font-semibold">+1 000 000 0000</p>
          <p className="text-sm text-zinc-700 font-semibold">irshad@gmail.com</p>
        </div>

      </div>
    </footer>
  );
};

export default Footer;
